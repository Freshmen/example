        ...
        (function(self) {
            ...
            self._getUUID(function(jqXHR, textStatus) {
                if(jqXHR.status != 200) {
                    ...
                    // store UUID
	            var uuidResponse = jqXHR.responseText;		
                    self.uuid = JSON.parse(jqXHR.responseText)["id"];
                    console.log("Parsed UUID: " + self.uuid);
                    // get SEBE ticket
                    self._getSEBETicket(function(
                        jqXHR, textStatus) {
                        complete(jqXHR.status,uuidResponse);   // sending back response text also in order to be able to get user name.
                    });
                }
            });
        })(this);
    ...
    Can.prototype.createFsioUserTicket = function(fsio, complete, ttl) {
        ...
        (function() {
            fsio.ticket.createWithCookies(function(jqXHR, textStatus) {
                ...
                complete(jqXHR.status, ticket,   jqXHR.responseText);
            }, params);
        })();
    };
